...
class UartUpperComputer:
    def __init__(self, root):
        self.root = root
        self.root.title("UART 上位机 - 升级app")
        ...
        # ===================== 升级区（自动识别A/B区） =====================
        self.frame_upgrade = ttk.LabelFrame(root, text="BIN 升级（自动识别AppA/B）")
    ...
    def auto_process(self):
        try:
            ...
            self.log("📤 发送：02 02 02 02（进入App）")
            time.sleep(0.1)

            # 2. 等待2s识别 A/B
            self.label_status.config(text="状态：等待AppA/B应答(2s)", foreground="blue")
            if self.wait_bytes(b'\xA5\xA5', 2):
                area = "AppA"
                self.flash_addr = 0x00018000
                self.flash_size = 0x00013800
            elif self.wait_bytes(b'\xB5\xB5', 2):
                area = "AppB"
                self.flash_addr = 0x0002B800
                self.flash_size = 0x00013800
            else:
