...
class UartUpperComputer:
    def __init__(self, root):
        self.root = root
        self.root.title("UART 串口上位机 (boot升级)")
    ...
    def auto_upgrade_process(self):
        try:
            ...
            # ===================== 先发进入boot指令 0x01 0x01 0x01 0x01 =====================
            self.label_status.config(text="状态：发送进入boot指令 0x01 0x01 0x01 0x01", foreground="blue")
            self.ser.write(b'\x01\x01\x01\x01')#update boot
            self.log("📤 自动发送：0x01 0x01 0x01 0x01 进入boot，等待1秒...")
            time.sleep(1)  # 等待2秒
